...
<nav class="top">
    <div class="top-bar">
        <span class="top-left">
            ...
            <c:if test="${!empty user}">
                ${user.name}
                <a href="http://localhost:8080/tmall/login" id="logout">退出</a>
            </c:if>
            <c:if test="${empty user}">
        ...
        <span class="pull-right">
           ...
           购物车<c:if test="${!empty user}"> <a href="cart"><span class=" glyphicon glyphicon-shopping-cart redColor"></span>
                <strong id="cart-number">0</strong>件</a></c:if>
